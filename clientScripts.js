...
function showChanceMsg() {
    ...
    if (data[lastId]["chance"]) data[id]["turn"] = false;
    if (data[id]["chance"] && data[id]["chance"] !=='') {} else return;
...
function showCommunityMsg() {
    ...
    if (data[lastId]["community"]) data[id]["turn"] = false;
    if (data[id]["community"] && data[id]["community"] !=='') {} else return;
...
function showBuildHouses() {
    for (var key in data) {
        if (data[key]["houseBuild"]) {
            var streetsAndHouses = data[key]["houseBuild"].split('e5');
            streetsAndHouses.forEach(function (item) {
                item = item.split('e1');
                //получаем место для домов из ячейки
                var street = item[0];
                var placeForHouses = getCellFromDataname(item[0]).querySelector('.houses');

                //если есть дома, то сначала удаляем все
                for (var i=0; i<placeForHouses.children.length; i++) {
                    placeForHouses.children[i].remove();
                }
                if (item[1] =='1hotel') {
                    ...
                    hotel.src = hotelSrc;
                    placeForHouses.appendChild(hotel);
                } else {
                    ...
                    for (var i =0; i < houses[0]; i++) {
                        ...
                        house.src = houseSrc;
                        placeForHouses.appendChild(house);
                    }
                }
            });
        }
